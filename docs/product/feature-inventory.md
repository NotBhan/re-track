...
| **FEAT-022** | Native Tree-sitter TypeScript / JS / JSX AST | AST & Code Intelligence | Internal | Production | `backend/app/services/repository_summary.py` (`TSGrammarCache`, `TSModuleResolver`, `TSCrossFileLinker`) | `tests/test_ts_js_parser.py`, `tests/test_ts_js_cross_file_graph.py`, `tests/test_ast_integrity.py` | Schema 2.0 / Parser 2.0.0; extracts functions, classes, interfaces, enums, JSX renders, and re-export barrel chains. |
...
| **FEAT-028** | Expanded Multi-Repository Retrieval Benchmark Suite | Benchmarking & Evaluation | Internal | Production | `backend/app/evaluation/expanded_benchmark.py`, `benchmarks/expanded/golden_tasks.json`, `benchmarks/corpus/` | `tests/test_expanded_benchmark.py`, `tests/test_expanded_benchmark_contract.py`, `tests/test_expanded_benchmark_incremental.py`, `tests/test_expanded_benchmark_reproducibility.py` | 36 golden tasks across 12 categories, 6 multi-language repositories, relationship-aware scoring, and 7 incremental mutation scenarios. |
